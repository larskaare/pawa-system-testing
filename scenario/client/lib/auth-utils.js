...
async function authenticateForTest(request, reply) {
    ...
    logger.warn('About to acquire token for test using username/password');
...
// Adding support for working behind a corp proxy
// The Support that came as part of msal-node 1.6 is yet not stable enough (23.02.22)
// https://github.com/AzureAD/microsoft-authentication-library-for-js/blob/dev/lib/msal-node/docs/faq.md#why-doesnt-my-app-function-correctly-when-its-running-behind-a-proxy
// https://github.com/AzureAD/microsoft-authentication-library-for-js/issues/2600

const proxyClient = require('./proxyclient.js');   //Add the proxy client
clientConfig.system.networkClient = proxyClient;   //Connect the proxy client to the msal config and replace the msal network adapter.
